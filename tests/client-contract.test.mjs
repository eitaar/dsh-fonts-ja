...
test("settings UI exposes three roles, source modes, and Japanese labels", async () => {
  ...
  assert.match(template, /設定 Settings/);
  assert.match(template, /日本語を表示 123/);
  assert.match(template, /const 日本語/);
...
test("font preview uses compact Japanese copy for a two-line card", async () => {
  const template = await readFile("lib/client.tpl.js", "utf8");
  assert.match(template, /value:\s*"設定 Settings"/);
  assert.match(template, /value:\s*"日本語を表示 123"/);
  assert.match(template, /value:\s*'const 日本語'/);
  assert.doesNotMatch(template, /日本語の文章を読みやすく表示します。Markdown/);
});
